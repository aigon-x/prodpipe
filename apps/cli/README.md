...
> Katalog aplikacji CLI AIGON Production Platform — narzędzie wiersza poleceń do zarządzania platformą.

## 1. Purpose
Cel: aplikacja CLI (narzędzie wiersza poleceń) do zarządzania platformą AIGON. Tu żyje kod, konfiguracja deklaratywna i manifesty aplikacji cli.
...
## 4. Contains
Kod źródłowy aplikacji CLI, manifesty wdrożeniowe, konfiguracja deklaratywna, testy, dokumentacja.

## 5. Does Not Contain
Brak runtime state (żyje w AIGON-X-FS), brak sekretów (tylko schematy/templates/referencje), brak drugiego Source of Truth.

## 6. Dependencies
Zależne od: `system/runtime`, `system/router`, `system/gateway`, `system/control-plane`, `system/identity`, `system/security`.
