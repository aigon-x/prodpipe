...
GATE_REGISTRY=(
  ...
  # drill (tools/resilience/restore-drill/), że migration 0014_resilience.sql istnieje.
  ...
  "$(printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' \
    ...
    'Resilience gate: weryfikuje warstwe odpornosci (HA/DR/Backup). Sprawdza ze config/registry.yaml ma klucze resilience (rto/rpo/restore_drill_max_age/dr_game_day_max_age + floors per tier), ze state.sh ma backup/restore/backup-restore-test, ze istnieje pipeline restore drill (tools/resilience/restore-drill/restore-drill.sh), ze migration 0014_resilience.sql istnieje. Checks RES-B-01..04: pokrycie backupu, restore drill, immutability, retention.' \
