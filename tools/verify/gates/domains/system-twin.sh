...
if [ -f "$STATE_DIR/migrations/0004_system_twin.sql" ]; then
  pass "SYSTEM-TWIN-001 migracja system_twin" BLOCKING "Migracja 0004_system_twin.sql obecna."
else
  fail "SYSTEM-TWIN-001 migracja system_twin" BLOCKING "Brak migracji 0004_system_twin.sql."
fi

# ── SYSTEM-TWIN-002: tabela system_twin_node w migracji ─────
if [ -f "$STATE_DIR/migrations/0004_system_twin.sql" ]; then
  if grep -qE 'CREATE TABLE.*system_twin_node' "$STATE_DIR/migrations/0004_system_twin.sql"; then
...
if [ -f "$STATE_DIR/migrations/0004_system_twin.sql" ]; then
  if grep -qE 'CREATE TABLE.*system_twin_edge' "$STATE_DIR/migrations/0004_system_twin.sql"; then
