...
# ── INVARIANT-002: migracje sekwencyjne (0001,0002,0003) ───
missing=0
for m in 0001_initial.sql 0002_history_hash.sql 0003_system_twin.sql; do
  if [ ! -f "$STATE_DIR/migrations/$m" ]; then
    missing=$((missing+1))
  fi
done
if [ "$missing" -eq 0 ]; then
  pass "INVARIANT-002 migracje sekwencyjne" BLOCKING "Migracje 0001-0003 obecne."
else
