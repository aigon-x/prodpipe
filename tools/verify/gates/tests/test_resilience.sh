...
#   T4: migration 0009_resilience.sql definiuje backup_catalog/
...
# --- T4: migration 0009 definiuje tabele resilience -------------------------
echo ""
echo "--- T4: migration 0009_resilience.sql definiuje tabele ---"
MIGRATION_0009="$ROOT/system/control-plane/state/migrations/0009_resilience.sql"
if [ -f "$MIGRATION_0009" ]; then
    if grep -qE 'CREATE TABLE.*backup_catalog' "$MIGRATION_0009" \
       && grep -qE 'CREATE TABLE.*resilience_requirements' "$MIGRATION_0009" \
       && grep -qE 'CREATE TABLE.*game_days' "$MIGRATION_0009" \
       && grep -qE 'CREATE TABLE.*spof_findings' "$MIGRATION_0009"; then
        t_pass "Migration 0009 definiuje backup_catalog/resilience_requirements/game_days/spof_findings"
    else
        t_fail "Migration 0009 NIE definiuje wszystkich wymaganych tabel"
    fi
else
    t_fail "Brak migration 0009_resilience.sql"
fi
