...
4. **Evidence jest zapisywany** — wyniki trafiają do tabeli `game_days` (migration 0014) oraz do katalogu evidence pipeline'u.
...
| Evidence wygenerowany | plik JSON + wpis w `game_days` | brak | `backup-restore-test` (F5) + migration 0014 |
...
Wyniki zapisujemy do tabeli `game_days` (migration 0014). Wpis zawiera:
...
| Tabela wyników | `system/control-plane/state/migrations/0014_resilience.sql` (tabela `game_days`) |
