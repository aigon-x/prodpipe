...
- [ ] Udokumentuj brak backupu jako **krytyczny finding** (tabela `spof_findings`, migration 0014) — to jest awaria procesu backupu, nie tylko awaria stanu.
...
### 5.1 Wpis do tabeli `game_days` (migration 0014)
...
### Sekcja 5 — Evidence
- [ ] Wpis do tabeli `game_days` (migration 0014).
...
| Tabela wyników / evidence | `system/control-plane/state/migrations/0014_resilience.sql` (tabele `backup_catalog`, `resilience_requirements`, `game_days`, `spof_findings`) |
